    ...
    for iface in results:
        ...
        if st(iface, "Bring up") != PASSED:
            ...
            if st(iface, "ethtool present") != PASSED:
                hidden.update(["Self-test supported", "ethtool -t"])

            if st(iface, "Self-test supported") != PASSED:
                hidden.add("ethtool -t")
            ...
            if st(iface, "Default route via iface") != PASSED:
                hidden.update([
                    "Ping gateway (IPv4)",
                    "Ping www.arm.com (IPv4)",
                    "wget and curl",
    ...
    try:
        ...
        for intrf in physical_ifaces:
            ...
            if have_ethtool:
                ...
                if "supports-test: yes" in result_test.stdout:
                    ...
                    try:
                        ...
                        if t.returncode == 0:
                            set_result(intrf, "ethtool -t", PASSED)
                        else:
                            first_line = next((ln for ln in (t.stdout + "\n" + t.stderr).splitlines() if ln.strip()), "")
                            set_result(intrf, "ethtool -t", WARNING, first_line or f"returncode={t.returncode}")
                    except subprocess.TimeoutExpired:
                        print_color("ethtool -t timed out (60s)", "WARN")
                        set_result(intrf, "ethtool -t", WARNING, "timeout")
                    ...
                    set_result(intrf, "ethtool -t", SKIPPED, "Self-test not supported")
                ...
                set_result(intrf, "ethtool -t", SKIPPED, "No ethtool")
            ...
            if has_ipv4 and has_default_route(intrf):
                set_result(intrf, "Default route via iface", PASSED)
            elif has_ipv4:
                reason = "No default route" + (" after DHCP" if has_dhcp else " (static config)")
                set_result(intrf, "Default route via iface", FAILED, reason)
            else:
                set_result(intrf, "Default route via iface", SKIPPED, "No IPv4 address")
            ...
            if results[intrf]["IPv4 address present"]["status"] != PASSED or \
               results[intrf]["Default route via iface"]["status"] != PASSED:
                skip_many(intrf, [
                    "Ping gateway (IPv4)",
                    "Ping www.arm.com (IPv4)",
                    "wget and curl",
            ...
            set_result(intrf, "Default route via iface", PASSED, f"gateway {gw}")
