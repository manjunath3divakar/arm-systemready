...
if [ $ADDITIONAL_CMD_OPTION != "noacs" ]; then
  ...
  cd /usr/bin
  python3 read_write_check_blk_devices.py </dev/null | tee /mnt/acs_results/linux_tools/read_write_check_blk_devices.log
  cd -
  ...
  cd /usr/bin
  python3 ethtool-test.py /mnt/acs_tests/config/system_config.txt | tee /mnt/acs_results/linux_tools/ethtool-test-temp.log
  cd -
  awk '{gsub(/\x1B\[[0-9;]*[JKmsu]/, "")}1' /mnt/acs_results/linux_tools/ethtool-test-temp.log > /mnt/acs_results/linux_tools/ethtool-test.log
  rm -f /mnt/acs_results/linux_tools/ethtool-test-temp.log
